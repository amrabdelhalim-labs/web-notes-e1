...
|--------|--------|-------------|
| `validateRegisterInput` | `RegisterInput` | username (3-30)، email، password (6+) |
| `validateLoginInput` | `LoginInput` | email valid، password (6+) |
| `validateNoteInput` | `NoteInput` | title (1-200)، type، content/audio بحسب النوع |
| `validateUpdateNoteInput` | `UpdateNoteInput` | title إن وُجد، audioDuration موجب |
| `validateUpdateUserInput` | `UpdateUserInput` | username/email/displayName/language إن وُجدت |
| `validateChangePasswordInput` | `ChangePasswordInput` | الحالية، الجديدة (6+)، التطابق، التغيير |
...
| `lib/apiErrors.ts` | أدوات API | ردود خطأ موحّدة بـ HTTP codes ورسائل عربية |
| `validators/index.ts` | تحقق المدخلات | دوال نقية → مصفوفة أخطاء، بدون جانب ضمني |
