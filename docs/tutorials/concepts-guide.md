...
GHCR هو مستودع صور الحاويات داخل GitHub. workflow ‎`docker-publish.yml`‎ يبني الصورة بعد اجتياز الاختبارات وفحص **Trivy**، ثم يدفعها إلى عنوان مثل ‎`ghcr.io/<مالك-المستودع-بأحرف-صغيرة>/web-notes-e1`‎. تُنشأ وسوم مثل اسم وسم الإصدار ‎`v*`‎ و‎`sha-<commit>`‎ وغالبًا ‎`latest`‎ عند دفع وسم إصدار. إن فشل السحب برسالة ‎manifest not found‎ فغالبًا الوسم غير موجود أو قديم؛ راجع صفحة **Packages** في GitHub أو أعد نشر إصدار ‎`v*`‎ بعد نجاح الـ workflow (تفاصيل في [deployment.md](../deployment.md) القسم 9).
